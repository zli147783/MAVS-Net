...
from utils.tools import np_skew_symmetric
...
class CorrespondencesDataset(Dataset):
    ...
    def process_image(self, image, aim_H=120, aim_W=160):
    ...
    def __getitem__(self, index):
        ...
        # Use dummy image paths for now
        img1_path = "dummy_path_1.jpg"
        img2_path = "dummy_path_2.jpg"

        # Use dummy images matching paper config (120x160), ViT will resize internally
        img1 = np.zeros((1, 3, self.config.image_H, self.config.image_W), dtype=np.float32)
        img2 = np.zeros((1, 3, self.config.image_H, self.config.image_W), dtype=np.float32)
        ...
        pts1_virt, pts2_virt = self.correctMatches(e_gt) # Corrected coordinates
        pts_virt = np.concatenate([pts1_virt, pts2_virt], axis=1).astype('float64')

        return {'K1': K1, 'K2': K2, 'R': R, 't': t, 'xs': xs, 'ys': ys, 'T1': T1, 'T2': T2, 'virtPt': pts_virt,
                'img1': img1, 'img2': img2, 'img1_path': img1_path, 'img2_path': img2_path, 'side': side}
...
def collate_fn(batch):
    ...
    data = {}
    dict_key = ['K1s', 'K2s', 'Rs', 'ts', 'xs', 'ys', 'T1s', 'T2s', 'virtPts', 'img1', 'img2', 'img1_path', 'img2_path', 'sides']
    for k in dict_key:
        data[k] = []
    ...
    for sample in batch:
        ...
        data['img1'].append(sample['img1'])
        data['img2'].append(sample['img2'])
        data['img1_path'].append(sample['img1_path'])
        data['img2_path'].append(sample['img2_path'])
